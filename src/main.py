...
lowObjgreen = np.array([52, 72, 107])  
upObjgreen = np.array([92, 134, 196])  
...
def object_(frame):  # Фукция для распознования кубиков
    ...
    # Забираем из изображения с камеры область интереса
    cube = frame[yObj1:yObj2, xObj1:xObj2]
    # Делаем размытие по гауссу для лучшего распознования
    Gauss = cv2.GaussianBlur(cube, (3, 3), cv2.BORDER_DEFAULT)
    # Переводим изображение из RGB в HSV
    hsv = cv2.cvtColor(Gauss, cv2.COLOR_BGR2HSV)
    # С помощью inRange мы находим маску красного на изображении
    rmask = cv2.inRange(hsv.copy(), lowObjred, upObjred)
    # С помощью inRange мы находим маску красного на изображении
    rmask2 = cv2.inRange(hsv.copy(), lowObjred1, upObjred1)
    # Складываем две маски карсного так как крансый может находится с двух сторон диапазона hsv
    rmask3 = cv2.bitwise_or(rmask, rmask2)
    max1 = 0
    # hsv1 = cv2.cvtColor(rmask3.copy(), cv2.COLOR_GRAY2BGR)
    # frame[yObj1:yObj2, xObj1:xObj2] = hsv1
    cv2.rectangle(frame, (xObj1, yObj1), (xObj2, yObj2),
                  (255, 0, 0), 2)  # Обводим датчик для кубиков
    # Находим контуры по маске rmask3
    rcnt,_ = cv2.findContours(
        rmask3, cv2.RETR_EXTERNAL, cv2.BORDER_DEFAULT)
    if len(rcnt) != 0:  # Проверка массива на наличие объектов
        for i in rcnt:
            ...
            if a1 > max1:  # Находим самый большой контур
                # Фультр, отсекает шумы по пллощади, заполнению контура и длине
                if a1 > 200 and a1 / (h1 * w1) > 0.5 and w1 < 270:
                    ...
                    if yred > 150 and Flag_test_red:  # Если кубик стал ниже половины датчика и Flag_test_red = True, то записываем в массив цвет кубика, время от кубика до линии и меняем индексы
                        ...
                        time_line_to_obj[per % 4][index_section] = round(
                            time.time() - time_section, 2)
                        index_section = 1
                        index_time = 1
                        Object_red += 1
                        Flag_test_red = False
                        Flag_index_red = False
                    # Если кубик стал выше половины и Flag_test_red = False, то поднимаем флаги для записи в массивы
                    if Flag_test_red == False and yred < 150:
                        Flag_test_red = True
                        Flag_index_red = True
                    yred_old = yred
                    # state = "Object"
                    ...
                    cv2.rectangle(cube, (x1, y1), (x1 + w1, y1 + h1),
                                  (0, 0, 255), 2)  # Обводим кубик
                else:
    ...
    # С помощью inRange мы находим маску зеленого на изображении
    gmask = cv2.inRange(hsv.copy(), lowObjgreen, upObjgreen)
    # Находим контуры по маске gmask
    gcnt,_ = cv2.findContours(gmask, cv2.RETR_EXTERNAL, cv2.BORDER_DEFAULT)
    max1 = 0
            ...
            if a1 > max1:  # Находим самый большой контур
                ...
                if a1 > 250 and a1 // (h1 * w1) < 0.6:
                    ...
                    if ygr > 150 and Flag_test_green:  # Если кубик стал ниже половины датчика и Flag_test_green = True, то записываем в массив цвет кубика, время от кубика до линии и меняем индексы
                        ...
                        time_line_to_obj[per % 4][index_time] = round(
                            time.time() - time_section, 2)
                        index_time = 1
                        index_section = 1
                        Object_green += 1
                    ...
                    # Если кубик стал выше половины и Flag_test_green = False, то поднимаем флаги для записи в массивы
                    if Flag_test_green == False and ygr < 150:
                    ...
                    Flag_test = False
                    cv2.rectangle(cube, (x1, y1),
                                  (x1 + w1, y1 + h1), (0, 255, 0), 2)
                else:
    ...
    # Если мы видим два кубика разных цветов, то мы будем ехать по кубику, который находится ближе (опкускаем флаг)
...
while 1:
    ...
    car.straight_forward(20) #เซฟโซนปลอดภัยๆ
    if ul<55 and e==0:
          ul_need_turn = True
