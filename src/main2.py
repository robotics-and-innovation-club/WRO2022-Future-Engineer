...
def grobject(frame):  
    global xObj1, xObj2, yObj1, yObj2, turn_red, turn_green, lowred, upred, lowred1, upred1, lowgreen, upgreen
   
    box = frame[yObj1:yObj2, xObj1:xObj2]

    Gauss = cv2.GaussianBlur(box, (3, 3), cv2.BORDER_DEFAULT)
   
    hsv = cv2.cvtColor(Gauss, cv2.COLOR_BGR2HSV)
   
    rmask = cv2.inRange(hsv.copy(), lowred, upred)
   
    rmask2 = cv2.inRange(hsv.copy(), lowred1, upred1)
    rmask3 = cv2.bitwise_or(rmask, rmask2)
    max1 = 0
    cv2.rectangle(frame, (xObj1, yObj1), (xObj2, yObj2),
                  (255, 0, 0), 2)  
    redr,_ = cv2.findContours(
    ...
    gmask = cv2.inRange(hsv.copy(), lowgreen, upgreen)
    
    greeng,_ = cv2.findContours(gmask, cv2.RETR_EXTERNAL, cv2.BORDER_DEFAULT)
    max1 = 0
    if len(gcnt) != 0:  
        for i in greeng:
            x1, y1, w1, h1 = cv2.boundingRect(i)  
...
def blue_tr():
    global e, ul, ul_need_turn, Flag_turn_red, Flag_turn_green
    if  ul_need_turn:
        car.left(40,25)
    elif Flag_turn_red:
        car.right(20,25)
    elif Flag_turn_green:
        car.left(20,25)
    elif e<0:
        car.left(15,15)
    elif e>40:
